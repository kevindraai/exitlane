...
async def create(endpoint,subnet="10.99.99.0/24",port=51820,interface="wg0",client="router",vpn_interface="nordlynx"):
    network=ipaddress.ip_network(subnet,strict=True); hosts=list(network.hosts())
    if network.version!=4 or len(hosts)<2: raise ValueError("invalid subnet")
    if not re.fullmatch(r"[A-Za-z0-9-]{1,15}",interface): raise ValueError("invalid interface")
    spubpriv=await keypair(); cpubpriv=await keypair(); spriv,spub=spubpriv; cpriv,cpub=cpubpriv
    server=f'''[Interface]
...
PersistentKeepalive = 25
'''
    WG_DIR.mkdir(parents=True,exist_ok=True,mode=0o700); (WG_DIR/f"{interface}.conf").write_text(server); (WG_DIR/f"{client}.conf").write_text(client_cfg); os.chmod(WG_DIR/f"{interface}.conf",0o600); os.chmod(WG_DIR/f"{client}.conf",0o600)
    return {"interface":interface,"server_public_key":spub,"client_public_key":cpub,"client_config":client_cfg,"client_name":client}
