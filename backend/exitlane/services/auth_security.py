...
def new_recovery_codes(connection: sqlite3.Connection, user_id: int, now: int) -> list[str]:
    codes = [
        f"{secrets.token_hex(4)}-{secrets.token_hex(4)}".upper() for _ in range(RECOVERY_CODE_COUNT)
    ]
...
def confirm_enrollment(user_id: int, session_token: str, enrollment: str, code: str) -> list[str]:
    ...
    with sqlite3.connect(core.DB, timeout=5.0) as connection:
        ...
        connection.execute(
            ...
            (user_id, token_hash(session_token)),
        )
    return codes
...
def list_sessions(user_id: int, current_id: str) -> list[dict]:
    ...
    return [
        {
            "id": row[0],
            "current": hmac.compare_digest(row[0], current_id),
            "created_at": row[1],
            "last_seen_at": row[2],
            "expires_at": min(row[3], row[4]),
            "client_ip": row[5],
            "user_agent": row[6],
        }
        for row in rows
