...
class Callback(BaseModel):
    callback_url: str = Field(min_length=20, max_length=2048)
...
async def setup_state():
    ...
    return {
        "complete": setting("setup_complete", False),
        "admin_created": count > 0,
        "provider": await provider.status(),
        "wireguard_configured": setting("wireguard_configured", False),
    }
...
async def login_token(req: Token):
    return await provider.login_token(req.token)


@app.post("/api/providers/nordvpn/login/callback")
async def login_callback(req: Callback):
    return await provider.login_callback(req.callback_url)


@app.post("/api/providers/nordvpn/configure-defaults")
async def defaults():
    return {"operations": await provider.defaults()}


@app.get("/api/providers/nordvpn/status")
async def status():
    return {"status": await provider.status()}


@app.get("/api/providers/nordvpn/countries")
async def countries():
    return {"countries": await provider.countries()}
...
async def wireguard(req: WireGuard):
    try:
        result = await create_wireguard(**req.model_dump())
...
async def webhook(req: Webhook):
    ...
    with sqlite3.connect(DB) as c:
        cur = c.execute("INSERT INTO webhooks(name,url) VALUES(?,?)", (req.name, req.url))
