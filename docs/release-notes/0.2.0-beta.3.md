...
Beta.3 appliance QA verifies the missing-tool UI and installs the pinned package on the designated
test appliance exclusively through the managed confirmation flow. It then verifies installation
status and CLI availability, with proof that no Speedtest measurement is invoked. No real Speedtest
is permitted.
