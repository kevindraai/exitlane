...
| Provider credential disclosure | Reusable NordVPN token or identifying Mullvad account number exposed through argv, output, logs, events, files, or responses | Nord private PTY; Mullvad fixed argv plus stdin; bounded mutable buffers wiped after classification; masked/cleared browser fields; safe codes and metadata | Sentinel argv/log/event/exception/API tests for each provider | A compromised browser, provider daemon, or root host can observe a live credential; real account use requires separate authorized validation |
...
| Provider package lifecycle changes appliance networking | Upstream post-install daemon or early-boot firewall start interrupts management before gateway policy is ready | APT-only `SYSTEMD_OFFLINE=1`; persistent ExitLane conditions gate both units before the package transaction; completion marker only after disconnected settings, route, and absent provider table are verified | Incident simulation, drop-in/digest/order regressions, live package reinstall monitor, interruption/reboot and Debian 13 appliance installation | `SYSTEMD_OFFLINE` and package scripts are version-sensitive and must be revalidated beyond systemd 257/Mullvad 2026.4; the package's required setuid `mullvad-exclude` remains allowed only in the fixed installer unit |
