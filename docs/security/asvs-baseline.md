...
| V2 authentication | yes | scrypt verifier, generic failures, TOTP MFA and one-time recovery codes | auth, replay and recovery-code regressions | TOTP remains phishable |
...
| V14 configuration | yes | environment/database precedence, explicit trusted-proxy CIDRs, debug off | config/proxy/unit tests | broad operator-configured trust remains dangerous |
