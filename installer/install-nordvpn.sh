...
validate_installation() {
  command -v nordvpn >/dev/null 2>&1 || die 68 "validation_failed"
  systemctl is-enabled --quiet nordvpnd || die 68 "validation_failed"
  systemctl is-active --quiet nordvpnd || die 67 "daemon_inactive"
}
...
main() {
  ...
  if command -v nordvpn >/dev/null 2>&1; then
    systemctl enable --now nordvpnd || die 67 "daemon_start_failed"
    validate_installation
    printf '%s\n' "installation_available"
    return
  fi

  local work_dir release_package
  work_dir="$(mktemp -d)"
